...
seeds_hist <- ggplot(seeds_long, aes(x = value )) +
  geom_histogram() +
  facet_grid(variety~name, scale = 'free')
...
ggplot(seeds_df)+
  geom_point(aes(x= p, y = l_k, color = asym, shape = variety,
                 size = 3, alpha = .7))
...
```{r}
#| fig-cap: we would select three groups intiate a kmeasn cluster because the kink in the graph is most pronounced at t3 clusters
...
```{r}
set.seed(10101)
seeds_km <- kmeans(x = seeds_scale, center = 3, nstart = 25) # kmeans specifying 3 groups to start
...
```{r}
#| tbl-cap: contiency table for the grouping of kmeans and the varetials of kernels
...
2. check for nas (none)
3. select only the numerics
4. scale the data
...
wb_ghg_20 <- wb_env |> slice_max(ghg, n=20)
